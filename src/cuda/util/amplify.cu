...
void amplify(thrust::host_vector<int> &values_to_amplify,
	...
	{
	thrust::host_vector<int> values_to_amplify_copy(values_to_amplify.size());
	thrust::host_vector<int> amplify_counts_copy(amplify_counts.size());

	/* these next 4 steps are needed to make sure that values with 0 amplifications are not in the final version */
	thrust::host_vector<int> viable_values;
	...
	thrust::host_vector<int> amplify_counts_offsets( number_of_values_to_amplify );
	thrust::inclusive_scan(amplify_counts_copy.begin(), amplify_counts_copy.end(), amplify_counts_offsets.begin()); 

	thrust::host_vector<int> temporary_sequence ( total_number_of_amplified_values );
...
void amplify_float(thrust::host_vector<float> &values_to_amplify,
		   thrust::host_vector<int> &amplify_counts,
	           thrust::host_vector<float> &amplified_values)
	{
	...
	thrust::host_vector<float> temporary_sequence ( total_number_of_amplified_values );
	thrust::sequence( temporary_sequence.begin(), temporary_sequence.end());
...
void amplify_sequence(thrust::host_vector<int> &amplify_counts,
		      ...
		      thrust::host_vector<int> &amplified_values)
	{
	thrust::host_vector<int> values_to_amplify(number_of_elements_in_sequence);
