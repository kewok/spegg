...
void DemeGeneticsSettings::read_in_data(const char *filename, int species_ID)
	...
	try
		...
		// std::cout << "FYI: No loci name. Use int indices to pass in genetics details " << std::endl;
		}
	...
	deme_specific_mutation_rates = new thrust::host_vector<float>[Number_of_Loci];
	deme_specific_mutation_magnitudes = new thrust::host_vector<float>[Number_of_Loci];
...
float *DemeGeneticsSettings::get_mutation_rates_ptr(int locus_index)
	{
	return(&deme_specific_mutation_rates[locus_index][0]);
	}

float *DemeGeneticsSettings::get_mutation_magnitudes_ptr(int locus_index)
	{
