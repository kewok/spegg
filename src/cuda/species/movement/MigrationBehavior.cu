...
//
// Include your migrate_mySpecies header file here
//

MigrationBehavior *MigrationBehavior::create_migrationBehavior(inds_stochastic_migratory *species)
	{
	/*
	* A generic template for the code that simulates the movement of individuals during each time step by reassigning them to new demes. This is to be populated by code generated by the user.
	*/

   // test for species id
  // 	return new MySpeciesMigration(inds_object[species_id], habitat);
	}
