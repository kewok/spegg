...
void DemeSettings_coevolvingSpecies ::read_in_interactions(const char *filename, int species_ID)
	...
	deme_wise_interaction_effects_on_fecundity = new thrust::device_vector<float>[interacting_species_indices.getLength()];
	deme_wise_interaction_effects_on_survivorship = new thrust::device_vector<float>[interacting_species_indices.getLength()];
