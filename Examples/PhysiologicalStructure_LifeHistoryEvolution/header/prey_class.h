...
class prey
	...
	public:
		...
		thrust::host_vector<float> prey_abundance;
		thrust::host_vector<float> effect_of_inds_on_prey;
		thrust::host_vector<float> prey_maximum_abundance;
		thrust::host_vector<float> prey_assimilation_efficiency;

		void specify_prey_properties_by_deme();

		void update_prey_abundance(thrust::host_vector<float> &effect_of_individuals_on_prey);

	protected:
		PreyDemeSpecificData *preyParameters;

		thrust::host_vector<float> prey_carrying_capacity;
		thrust::host_vector<float> prey_unconstrained_growth_rates;
		thrust::host_vector<float> prey_growth_rate_noise_stddev;
